...
```
TOTAL JOBS 43   ·   PASSED GATES 28   ·   NOT APPLIED 43   ·   APPLIED 0
```

Gate breakdown — all four rule types now firing:

| Reason | Count |
|---|---|
| requires 4+ years, above the 3-year limit | 4 |
| title contains blocked word "sr" | 3 |
| title contains blocked word "iii" | 2 |
| requires 8+ years, above the 3-year limit | 2 |
| requires 5+ years, above the 3-year limit | 1 |
| title contains blocked word "consultant" | 1 |
| title contains blocked word "specialist" | 1 |
| location [Newcastle] outside target geography | 1 |
...
### Enrichment keyed on Job ID instead of the apply link
`enrichPending()` parsed the requisition id and skipped anything that fell back to
a content hash — which is most Workday rows. So the majority of the sheet was
never enriched, the experience gate had no JD text to read, and it passed every
row silently. **Not one rejection in 43 rows was on years of experience.**

**Fix:** `fetchJdByUrl()` works from the apply link alone and adds adapters for
**Workday** (the undocumented `/wday/cxs/` endpoint, both URL shapes) and
**Amazon** (`.json` on any job URL), alongside Greenhouse, Lever, Ashby and
SmartRecruiters. Greenhouse now also resolves bare `?gh_jid=` links using Company
Name as the board hint.

**Result:** 7 rows rejected on experience, including the highest-scoring job in
the sheet (Hach, match 97, wants 4+ years). A ranking-only system would have put
that one first.

Note `jdExperienceFloor()` takes the **minimum** years figure in the JD, so it is
deliberately lenient: "2–5 years" passes, "5–8 years" rejects.
...
**Short**
1. Second Spark schedule aimed at Lever/Greenhouse/Ashby startups, so the two
   schedules do not return the same companies.
2. Fix dead slugs: `doordash`, `sentry`, `Visa`, `Bosch` (the last two return 0
   postings rather than 404 — wrong SmartRecruiters company name).
3. Extend `CANDIDATE_TENANTS` and re-run `findTenants`. Hit rate was ~21%, so a
   longer candidate list is the cheapest way to grow coverage.

**Then — Phase 4, the applier**
4. Playwright CLI with a persistent browser profile.
5. Per-ATS form adapters. Greenhouse, Lever and Ashby share a similar shape and
   together cover Unacademy, Tekion, Mindtickle, Meesho, HighRadius. Workday is
   its own project and needs an account per tenant.
6. Google SSO for boards that demand a login.
7. Write `Applied` + `Applied At` back through `doPost`.
8. **Dry-run mode first** — fill every field, screenshot the completed form, stop
   short of submit. The first real applications get eyeballed before anything goes
   out under Vinoth's name.
9. The scraper also unlocks the employers with no public API (section 4).
